...
func BulkForward(caller, url, user, pass string, toSend []ElkBulkUpdateItem) {
	...
	for i := range toSend {
		...
		// Validate document to prevent empty field names
		if toSend[i].Doc != nil {
			// Convert to JSON then back to map to check for empty keys
			docJSON, err := json.Marshal(toSend[i].Doc)
			if err != nil {
				log.L.Errorf("%v Couldn't marshal document for validation: %v", caller, err)
				continue
			}

			// Check for empty string patterns in JSON and replace them
			if bytes.Contains(docJSON, []byte(`"":`)) || bytes.Contains(docJSON, []byte(`"": `)) {
				log.L.Warnf("%v Auto-generating key - Thanks Danny!!!: %s", caller, docJSON)

				// Replace empty keys with "AutoGenerated"
				docJSON = bytes.Replace(docJSON, []byte(`"":`), []byte(`"AutoGenerated":`), -1)
				docJSON = bytes.Replace(docJSON, []byte(`"": `), []byte(`"AutoGenerated": `), -1)

				// Unmarshal the fixed JSON back into the original document
				var fixedDoc interface{}
				if err := json.Unmarshal(docJSON, &fixedDoc); err != nil {
					log.L.Errorf("%v Couldn't unmarshal fixed document: %v", caller, err)
					continue
				}
	...
	if validItems == 0 {
		log.L.Warnf("%v No valid items to send after validation, skipping bulk update", caller)
		return
	}

	//once our payload is built
	log.L.Debugf("%v Payload built, sending %d valid items...", caller, validItems)
