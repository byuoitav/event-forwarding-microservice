...
func GetDefaultHumioForwarder(interval time.Duration, bufferSize int, ingestToken string) *HumioForwarder {
	infoMsg := fmt.Sprintf("Getting default Humio forwarder with bufferSize %d and interval %s", bufferSize, interval.String())
	slog.Info(infoMsg)
...
func (e *HumioForwarder) Send(toSend interface{}) error {
	...
	default:
		TypeError := &customerror.StandardError{
			Message: fmt.Sprintf("Invalid type to send via a Humio Forwarder, must be an event that matches the byuoitav events package"),
		}
		return TypeError
	}
...
func (e *HumioForwarder) bufferevent(event events.Event) {
	infoMsg := fmt.Sprintf("Buffering event from %s for Humio\n", event.GeneratingSystem)
	slog.Debug(infoMsg)
	...
	if len(e.eventsBuffer) > e.bufferSize-1 {
		debugMsg := fmt.Sprintf("Humio Buffer surpassing %d events", e.bufferSize)
		slog.Debug(debugMsg)
		e.sendBuffer()
		e.flushBuffer()
	}
...
func (e *HumioForwarder) flushBuffer() {
	infoMsg := fmt.Sprintf("Flushing buffer of %d events for Humio", len(e.eventsBuffer))
	slog.Debug(infoMsg)
...
func (e *HumioForwarder) marshalBuffer() []byte {
	...
	if err != nil {
		slog.Debug("Failed to marshal buffer for Humio", err.Error())
	}
	slog.Debug(string(logs))
	return logs
}

// send the buffer to humio
func (e *HumioForwarder) sendBuffer() error {
	infoMsg := fmt.Sprintf("Sending buffer for Humio of %d events", len(e.eventsBuffer))
	slog.Info(infoMsg)
	if len(e.eventsBuffer) > 0 {
		_, err := humio.MakeHumioRequest(http.MethodPost, "/api/v1/ingest/json", e.marshalBuffer(), e.ingestToken)
		if err != nil {
			slog.Debug("Failed to send buffer for Humio", err.Error())
			return err
		}
	}
	return nil
