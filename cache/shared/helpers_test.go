...
func TestSetDeviceField(t *testing.T) {
	...
	if err != nil {
		slog.Warn("SetDeviceField error", "err", err)
		t.Error(err.Error())
		t.FailNow()
	}
	assert.Equal(t, true, update)
	...
	new2, update, err := EditDeviceFromEvent(eGood, new)
	if err != nil {
		slog.Warn("EditDeviceFromEvent error", "err", err)
		t.Error(err.Error())
		t.FailNow()
	}
	new = new2
	...
	new2, update, err = EditDeviceFromEvent(eBad, new)
	if err != nil {
		slog.Warn("EditDeviceFromEvent error", "err", err)
		t.Error(err.Error())
		t.FailNow()
	...
	new2, update, err = EditDeviceFromEvent(eLate, new)
	if err != nil {
		slog.Warn("EditDeviceFromEvent error", "err", err)
		t.Error(err.Error())
		t.FailNow()
...
func BenchmarkUpdateDevice(b *testing.B) {
	// Set slog to discard output for benchmarks
	h := slog.NewTextHandler(nil, nil)
