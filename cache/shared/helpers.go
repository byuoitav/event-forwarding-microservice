...
func updateHeartbeat(v events.Event, c Cache) (bool, *nerr.E) {
	split := strings.Split(v.GeneratingSystem, "-")
	if len(split) < 3 {
		return false, nerr.Create("invalid generating system", "invalid-arguments")
	}
	heartbeatEvent := events.Event{
		GeneratingSystem: "",
		Timestamp:        v.Timestamp,
		EventTags:        []string{events.Heartbeat, events.AutoGenerated},
	...
	return ForwardAndStoreEvent(heartbeatEvent, c)
}

//ForwardAndStoreEvent .
func ForwardAndStoreEvent(v events.Event, c Cache) (bool, *nerr.E) {
	if len(v.GeneratingSystem) > 0 {
		_, err := updateHeartbeat(v, c)
		if err != nil {
			return false, nil
		}
